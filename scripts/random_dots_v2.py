...
class ParticleBox:
    ...
    def random_dots_l(self, dt, index_direction_l):
        ...
        self.state[crossed_x1, 0] =   np.random.uniform(-3.2,3.2) #self.bounds[0] + self.size
        self.state[crossed_x2, 0] =   np.random.uniform(-3.2,3.2) #self.bounds[1] - self.size

        self.state[crossed_y1, 1] =   np.random.uniform(-3.2,3.2) #self.bounds[2] + self.size
        self.state[crossed_y2, 1] =   np.random.uniform(-3.2,3.2) #self.bounds[3] - self.size
...
nparticle_c = np.floor((nparticles*coherence_level))
...
plt.hold(True)
particles_r, = ay.plot([], [], ms=0.2, markersize=2, markerfacecolor='w', markeredgecolor='k', marker='o',linestyle='')
...
def onClick(event):
    ...
    global index_direction_l
    global index_direction_r


    if(len(index_direction_r) > 0):
        coherence_level  = coherence_level +0.01
        index_direction_l = np.append(index_direction_l,index_direction_r[0])
        index_direction_r = index_direction_r[1::]
    if(len(index_direction_r) == 0):
        #coherence_level  = coherence_level 
        tmp_r = index_direction_r
        tmp_l = index_direction_l 
        index_direction_r = tmp_l
        index_direction_l = tmp_r


    print coherence_level
    print index_direction_l
    print index_direction_r
...
def animate(i):
    ...
    if(swap == 1):
        box.random_dots_r(dt,index_direction_l)
        box.random_dots_l(dt,index_direction_r)        
