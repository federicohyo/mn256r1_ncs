...
class Viewer(wx.PySimpleApp):

        '''Class to monitor and display aer activity'''
    
        def __init__(self, channel=0, port=50001, host='localhost', fps = 25,
                     marker='circle', markersize=4, color='black'):
                '''Aer2DViewer class is used to display AER activity in two dimentions
                   channel : channel number.
                   host : Aex server.
                   dims : Range of neurons to be displayed.
                   fps : frames per second.
                   Aer2DViewer(channel=0, host='localhost', dims = (64,32), fps = 40)
                '''
        ...
        def show(self):
                ...
                '''
                from Updater1D import EventsUpdater, LearningGUI
                self.gui = LearningGUI(marker=self.marker, markersize=self.markersize,
                                     color=self.color)
                self.eventsUpdate = EventsUpdater(self.gui, 
                                                  host=self.host, 
                                                  port=self.port,
