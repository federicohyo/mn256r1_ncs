...
class Retina_cx():
    ...
    def map_cx_output_layer_to_mn256r1(self, n_columns=256):
    
        pre_addresses = np.array(np.linspace(0,255,256)) #zero address
        ...
        for this_col in range(n_columns):
            ...
            post_addresses.append([this_coladdr-2**21,this_coladdr-2**21])
        ...
        memoffset = self.mem_offset_retina+10
        for i in range(n_columns):
            memoffset = self._program_multicast(pre_addresses[i], np.array(post_addresses[i]), memoffset, self.mem_offset_retina, last4bits)
            
                  
        #self.mapper._Mappings__close_device()  ############################ CLOSE
        self.mapper._program_detail_mapping(2**self.retina_interface+2**self.mn256r1_interface)
    ...
    def _init_fpga_mapper(self):
        ...
        self.mapper._program_bulk_spec(self.retina_interface,2**self.mn256r1_interface)#2**self.usb_interface)#2**self.usb_interface)
        ...
        self.mapper._program_memory(self.mem_offset_retina+self.mn256r1_interface,133120,0,prob=1) # memoffset taget interface, shift, 0
