...
def create_broadcast_matrices(n_classes=2, overlap=0.0):
    '''
    this function create distinc classes of stimuli (by returning the broadcast matrix)
    it uses the broadcast address
    256 perceptrons should be used
    '''
    a = range(256)
    syn = inputpop[a].synapses['broadcast'][0::256]
    #let's create the classes 
    matrix_pattern = np.zeros([256,256])
    for i in range(256):
        if(i < 128):
            matrix_pattern[i,:] = 0
        else:
...
def build_broadcast_stimulus_with_teacher(fpre = 100,  fteacher=50,duration=1000):
    '''
    this function creates broadcast stimulus with teacher directed to matrix_b syn
    '''       
    a = range(256)
    syn = inputpop[a].synapses['broadcast'][0::256]
    broadcast_stim = syn.spiketrains_poisson(fpre, duration=duration)
    #now build up a teacher signal
    syn_teaching_this_neu = inputpop.synapses['virtual_exc']
    teacher_sign = syn_teaching_this_neu.spiketrains_poisson(fteacher, duration=duration)
    stim = pyNCS.pyST.merge_sequencers(teacher_sign,broadcast_stim)
    return stim

####################################################################################
# simple multi-perceptron experiment, we use 256 parallel perceptrons
# showing at which degree of overlap we can still binary classify
####################################################################################
...
#program broadcast learning syn
nsetup.chips['mn256r1'].load_parameters('biases/biases_setbroadcast.biases')
matrix_b = create_broadcast_matrices()
nsetup.mapper._program_onchip_broadcast_learning(matrix_b)
...
for i in range(ntrial):
    ...
    nsetup.chips['mn256r1'].load_parameters('biases/biases_wijlearning_tbiocas15.biases')
    time.sleep(0.2)
    stim = build_broadcast_stimulus_with_teacher(fpre = 100,fteacher=50,duration=duration) 
    ...
    subplot(ntrial,1,i)
    imshow(syn_state)
    print "syn pot total ", np.sum(syn_state)
    raw_input("daje")
